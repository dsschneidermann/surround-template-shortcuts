...
namespace LiveTemplateShortcuts.GotoWordSample
{
    ...
    {
        [CanBeNull]
        private readonly IPsiSourceFile myCurrentFile;

        [CanBeNull]
        private readonly LocalOccurrencesHighlighter myHighlighter;

        [NotNull]
        private readonly IProjectModelElement myProjectElement;
        ...
        public GotoWordController([NotNull] Lifetime lifetime, [NotNull] IShellLocks shellLocks, [NotNull] IProjectModelElement projectElement, [CanBeNull] ITextControl textControl,
            ...
            : base(lifetime, new GotoByNameModel(lifetime), shellLocks)
        {
            ...
            if (projectFile != null)
            {
                ...
                if (textControl != null)
                {
                    myHighlighter = new LocalOccurrencesHighlighter(lifetime, shellLocks, textControl, markupManager);
        ...
        [NotNull]
        public IProperty<object> SelectedItem { get; }

        private void AdviceSelectionItem([NotNull] PropertyChangedEventArgs<object> args)
        {
            if (!args.HasNew)
            {
                return;
        ...
        {
            model.IsCheckBoxCheckerVisible.FlowInto(lifetime, model.CheckBoxText, flag => flag ? "Middle match" : string.Empty);
        ...
        protected override bool UpdateItems(string filterString, Func<IEnumerable<JetPopupMenuItem>, AddItemsBehavior, bool> itemsConsumer)
        {
        ...
        private static IEnumerable<LocalOccurrence> SearchInCurrentFile([NotNull] string searchText, [NotNull] IPsiSourceFile sourceFile)
        {
            ...
            while ((offset = fileText.IndexOf(searchText, offset, StringComparison.OrdinalIgnoreCase)) >= 0)
            {
                ...
                yield return new LocalOccurrence(documentRange, documentLine, foundText, leftFragment, rightFragment);
        ...
        private class Foo1 : MenuItemDescriptor
        {
                ...
                : base(displayName + ":" + occurrence.LineNumber)
            {
                ...
                ShortcutText =
                    RichText.Empty.Append(occurrence.LeftFragment, TextStyle.FromForeColor(SystemColors.GrayText))
    ...
    internal sealed class LocalOccurrence
    {
        public LocalOccurrence(DocumentRange range, int lineNumber, string foundText, string leftFragment, string rightFragment)
        {
            Range = range;
        ...
        public string LeftFragment { get; }
        public string RightFragment { get; }
    }

    internal static class EnumerableEx
    {
        [NotNull]
        public static IList<T> TakeFirstAndTail<T>([NotNull] this IEnumerable<T> source, int count, [NotNull] out IEnumerable<T> tailItems)
        {
