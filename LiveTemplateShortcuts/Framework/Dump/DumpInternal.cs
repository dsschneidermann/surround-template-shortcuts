...
using LiveTemplateShortcuts.Framework.Dynamic;
using LINQPad;
    ...
    internal static class DumpInternal
    {
                                     ...
                                     [CallerLineNumber] int linenumber = -1, [CallerMemberName] string caller = null)
        {
            ...
            if (objType.IsCOMObject)
            {
                ...
                //    stringOut(new DumpOutput(linePrefix, $"Exception in Dump{descriptionLine}. {ex.GetType().Name}: {ex.Message}", null, typeof(T).Name, null));
                //    return;
                //}
            }
